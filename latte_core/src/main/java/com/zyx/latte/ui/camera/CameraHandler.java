...
public class CameraHandler implements View.OnClickListener {
    ...
    private void takePhoto() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            Log.d("camera_handler_uri", uri.toString());
            //需要将Uri路径转化为实际路径
            final File realFile = FileUtils.getFileByPath(FileUtil.getRealFilePath(DELEGATE.getContext(), uri));
            Log.d("camera_realFile",realFile.toString());
            final Uri realUri = Uri.fromFile(realFile);
            CameraImaegBean.getInstance().setPath(realUri);
            intent.putExtra(MediaStore.EXTRA_OUTPUT, uri);
        } else {
            final Uri fileUri = Uri.fromFile(tempFile);
            CameraImaegBean.getInstance().setPath(fileUri);
