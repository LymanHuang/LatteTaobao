...
public class ImageDelegate extends LatteDelegate {
    ...
    private void initImages() {
        ...
        final ArrayList<MultipleItemEmity> entities = new ArrayList<>();
        final int size;
        if (pictures != null) {
            size = pictures.size();
            for (int i = 0; i < size; i++) {
                final String imageUrl = pictures.get(i);
                final MultipleItemEmity entity = MultipleItemEmity
                        .builder()
            ...
            final RecylerImageAdapter adapter = new RecylerImageAdapter(entities);
